...
│
├── js/
│   ├── constants.js          카테고리/상태/지역/요일 메타데이터
...
│   ├── restaurantWrite.js     맛집 등록 / 평점 남기기 (Firestore 쓰기)
...
## 4. 앱에서 맛집 등록하기 / 평점 남기기

- 상단 **"➕ 맛집 등록"** → 폼 작성 → 저장하면 바로 Firestore에 쓰이고, 지도/리스트에
  즉시 반영됩니다.
- 이미 등록된 맛집을 클릭 → 상세 패널의 **"평점 남기기"**에서 별점을 고르고
  등록하면, 여러 명이 매긴 점수의 **평균**으로 "우리 평가"에 표시됩니다.
  (개별 평점 값이 아니라 합계/횟수를 원자적으로 누적하는 방식이라 동시에
  여러 명이 눌러도 안전합니다.)
- 이름/좌표/상태처럼 맛집 자체의 정보를 고치는 기능은 아직 앱에 없습니다.
  Firestore 콘솔에서 직접 수정해주세요.
